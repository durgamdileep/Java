...
## 🔒 LinkedHashMap and Concurrency

- ⚠️ **Not thread-safe:**  
  `LinkedHashMap` is **not synchronized**, so it is not safe to use from multiple threads without external synchronization.

- ❗ **ConcurrentModificationException:**  
  If multiple threads access and modify a `LinkedHashMap` concurrently without proper synchronization, it can throw a `ConcurrentModificationException`.


### 🛠️ How to handle concurrency:
